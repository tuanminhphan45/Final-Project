...
class MSRVTTCapBuilder(BaseDatasetBuilder):
    ...
    DATASET_CONFIG_DICT = {
        "default": "/storage/student10/vidcaption/LAVIS/lavis/configs/datasets/msrvtt/defaults_cap.yaml",
    }
    def build(self):
        """
        Build MSRVTT dataset for video captioning.
        Returns:
            Dict of train, val, test datasets.
        """
        # Xây dựng processors (video và text)
        self.build_processors()
        
        # Lấy thông tin từ config
        build_info = self.config.build_info
        
        # Khởi tạo dict để lưu datasets
        datasets = dict()

        # Build datasets cho mỗi split (train/val/test)
        ...
        for split in splits:
            ...
            # Lấy đường dẫn annotation
            ann_path = build_info.annotations[split].storage
            if not isinstance(ann_path, list):
                ann_path = [ann_path]
            
            # Lấy đường dẫn video
            vis_path = build_info.videos.storage
